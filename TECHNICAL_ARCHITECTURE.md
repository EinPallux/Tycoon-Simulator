...
- **Coaster tracks (as built, Phase 3 — `render/CoasterLayer.tsx`):** instanced per piece-model with raw matrices (tracks anchor at tile-edge nodes with height offsets, so the entity-footprint instancer doesn't apply). Kenney CoasterKit conventions verified against GLB bounds: pieces author entering the origin heading +z (= sim dir S); single-tile `-track` pieces are tile-centered at deck y 0–0.1; corners/hills/loops anchor at the entry edge one unit below the origin (place at `h+1`). **Chirality trick:** the kit's one corner model is a right turn — a *left* corner renders the same model anchored at its exit facing backward; slope-down likewise reuses the slope model exit-anchored. Supports stack unit columns under elevated node points. **Trains** are per-coaster mesh groups positioned by `pointOnPiece` — elapsed run time → arc length (cars space by arc, not time, so they stay coupled across chain/gravity speed jumps), with an analytic radial up-vector through loops ((0,1,0) degenerates at the vertical tangents). Draft ghosts reuse the same placement math with a shared transparent material.
- **Weather grading (Phase 3):** `SkyAndLights` damps a per-kind {sun, ambient, gray} grade (≈2 s time constant) into the palette-curve output — fronts roll in rather than snap; rain is a camera-following points field, gated by intensity.
- **Zones & juice (Phase 4):** themed zones are DERIVED sim state — union-find clustering re-runs after any placeable change (edit-rate) and on load; banners are canvas-texture sprites (no text libs, no font fetches). All celebration particles share one pooled 320-slot instanced mesh (SoA typed arrays, swap-remove compaction, zero per-frame allocation). The onboard cam reuses the train's arc-length locator and simply takes the camera while active (the RTS rig yields). Photo mode overrides only the *lighting* time — the sim never pauses for a photograph.
- **Music (Phase 4, `audio/music.ts`):** fully procedural chip-orchestra — a 110 ms lookahead scheduler walks seeded pentatonic patterns per mood (menu/day/night/storm) into a dedicated `musicGain`; mood switches crossfade. Render-side PRNG only — the sim's rng is never touched. Documented deviation from "sourced CC0 packs" (ASSET_GUIDE §5): zero assets, zero license surface, and the soundtrack reacts to weather/time instantly.
