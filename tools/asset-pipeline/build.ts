...
async function findFileInKit(kit: string, file: string): Promise<string> {
  ...
  async function walk(dir: string): Promise<void> {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      else if (entry.name === file) matches.push(p);
    }
  }
  await walk(kitDir);
...
function generateManifest(models: BuiltEntry[], skies: BuiltEntry[]): string {
  const modelLines = models
    .map((m) => `  "${m.id}": { url: "${m.url}", kit: "${m.kit}" },`)
    .join("\n");
  const skyLines = skies.map((s) => `  "${s.id}": { url: "${s.url}", kit: "${s.kit}" },`).join("\n");
  const kits = [...new Set([...models, ...skies].map((e) => e.kit))].sort();
  return `/**
...
async function main(): Promise<void> {
  ...
  await writeFile(MANIFEST_OUT, generateManifest(models, skies));

  const total = [...models, ...skies].reduce((sum, e) => sum + e.bytes, 0);
  const report = [...models, ...skies]
    .sort((a, b) => b.bytes - a.bytes)
    .slice(0, 10)
    .map((e) => `  ${(e.bytes / 1024).toFixed(1).padStart(8)} KB  ${e.id}`)
    .join("\n");
  console.log(`asset-pipeline: ${models.length} models + ${skies.length} skies`);
  console.log(`largest:\n${report}`);
